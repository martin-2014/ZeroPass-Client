import component from './zh-CN/component.json';
import menu from './zh-CN/menu.json';
import pages from './zh-CN/pages.json';
import login from './zh-CN/login.json';
import activate from './zh-CN/activate.json';
import register from './zh-CN/register.json';
import limit from './zh-CN/limit.json';
import common from './zh-CN/common.json';
import multiselect from './zh-CN/multiselect.json';
import authorization from './zh-CN/authorization.json';
import myapps from './zh-CN/myapps.json';
import overview from './zh-CN/overview.json';
import app from './zh-CN/app.json';
import userProfile from './zh-CN/userProfile.json';
import about from './zh-CN/about.json';
import errors from './zh-CN/errors.json';
import closeOption from './zh-CN/closeOption.json';
import notification from './zh-CN/notification.json';
import settings from './zh-CN/settings.json';
import country from './zh-CN/glossaries.json';
import tip from './zh-CN/tip.json';
import password from './zh-CN/password.json';
import wallet from './zh-CN/wallet.json';
import sync from './zh-CN/sync.json';
import dataStorage from './zh-CN/dataStorage.json';

export default {
    ...menu,
    ...component,
    ...pages,
    ...login,
    ...activate,
    ...register,
    ...limit,
    ...common,
    ...multiselect,
    ...authorization,
    ...myapps,
    ...overview,
    ...app,
    ...userProfile,
    ...about,
    ...errors,
    ...closeOption,
    ...notification,
    ...settings,
    ...country,
    ...tip,
    ...password,
    ...wallet,
    ...sync,
    ...dataStorage,
};
