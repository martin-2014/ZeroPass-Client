import component from './it-IT/component.json';
import menu from './it-IT/menu.json';
import pages from './it-IT/pages.json';
import login from './it-IT/login.json';
import activate from './it-IT/activate.json';
import register from './it-IT/register.json';
import limit from './it-IT/limit.json';
import common from './it-IT/common.json';
import multiselect from './it-IT/multiselect.json';
import authorization from './it-IT/authorization.json';
import myapps from './it-IT/myapps.json';
import overview from './it-IT/overview.json';
import app from './it-IT/app.json';
import userProfile from './it-IT/userProfile.json';
import about from './it-IT/about.json';
import errors from './it-IT/errors.json';
import closeOption from './it-IT/closeOption.json';
import notification from './it-IT/notification.json';
import settings from './it-IT/settings.json';
import country from './it-IT/glossaries.json';
import tip from './it-IT/tip.json';
import password from './it-IT/password.json';
import wallet from './it-IT/wallet.json';
import sync from './it-IT/sync.json';
import dataStorage from './it-IT/dataStorage.json';

export default {
    ...menu,
    ...component,
    ...pages,
    ...login,
    ...activate,
    ...register,
    ...limit,
    ...common,
    ...multiselect,
    ...authorization,
    ...myapps,
    ...overview,
    ...app,
    ...userProfile,
    ...about,
    ...errors,
    ...closeOption,
    ...notification,
    ...settings,
    ...country,
    ...tip,
    ...password,
    ...wallet,
    ...sync,
    ...dataStorage,
};
